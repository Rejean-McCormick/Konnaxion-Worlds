...
class HarvestApp:
    def __init__(self) -> None:
        self.root = tk.Tk()
        self.root.title("Konnaxion — Bug Harvest Wave 2")
        self.root.geometry("1080x720")
        self.root.minsize(900, 600)

        self.run_backend = tk.BooleanVar(value=False)
        self.run_frontend = tk.BooleanVar(value=True)
        self.status = tk.StringVar(value="Prêt — Wave 2 ciblée, aucun full suite.")
    ...
    def _build(self) -> None:
        ...
        ttk.Label(
            frame,
            text="Konnaxion — Bug Harvest Wave 2",
            font=("Segoe UI", 16, "bold"),
        ).pack(anchor="w")
        ttk.Label(
            frame,
            text=(
                "Wave 2 : keenKonnect + Kreative + Kontrol — runtime/API + source gaps. "
                "Pas de Wave 1, pas de pytest complet, pas de build, pas de route-smoke global."
            ),
        ...
        ttk.Checkbutton(
            opts,
            text="Wave 1 backend — déjà vert (désactivé)",
            variable=self.run_backend,
            state="disabled",
        ).pack(anchor="w")
        ttk.Checkbutton(
            opts,
            text="Playwright Wave 2 — keenKonnect + Kreative + Kontrol (Next :3001)",
    ...
    def start(self) -> None:
        ...
        self.run_dir = ROOT / ".bug-harvest-wave2-runs" / stamp
    ...
    def _worker(self) -> None:
        ...
        self.emit("=== TARGETED BUG HARVEST WAVE 2 ===")
        ...
        try:
            ...
            if self.run_frontend.get() and not self.stop_event.is_set():
                ...
                code = self._run(
                    ...
                    self._cmd_for_executable(
                        ...
                        [
                            ...
                            "--project=platform-harvest-wave2",
                        ],
                    ),
                    FRONTEND,
                    timeout=420,
                ...
                results.append(("frontend-wave2", "PASS" if code == 0 else "FAIL", code))
                completed += 1
                self._progress(completed, selected)
