...
> **Implementation status — 2026-09-08:** `KX-WORLDS-1` has control-plane models, schema-per-WorldRelease runtime isolation, World-aware routing, SeedPack builds, personas, EkoH/Smart Vote scoping, snapshots, rollback-by-new-release, frontend World switching, and architecture checks implemented in the repository snapshot reviewed on 2026-09-08. Runtime PostgreSQL/Django integration validation and production-scale acceptance remain release gates before deployment.

> **Production target — v1.1.0:** one logical Konnaxion deployment, initially suitable for one server/VPS when capacity permits, hosting approximately **120 registered switchable Worlds**. World switching is ordinary request/navigation routing and never starts/stops per-World infrastructure or triggers build/import/reset/provision work.
...
Implementation state: **implemented / pending runtime and production-scale validation**
...
The production catalog target is approximately **120 registered Worlds** served by the same logical application stack. This target does not mean 120 repositories, 120 Docker stacks, or 120 server-global active contexts.
...
2. `01_CURRENT_STATE_BASELINE.md` — observed baseline and migration context.
3. `02_GLOSSARY.md` — exact meanings of World, Seed Pack, Release, Snapshot, Principal, Persona, etc.
4. `03_ARCHITECTURE.md` — control plane, schemas, runtime topology and one-server baseline.
...
7. `06_RUNTIME_CONTEXT_AND_ROUTING.md` — URL, request scope, database scope, response headers and switch path.
...
10. `09_INFRASTRUCTURE_SCOPING.md` — cache, Celery, WebSocket, search, embeddings, media, job and health scaling.
11. `10_CONTROL_PLANE_API.md` — control and runtime API contracts.
12. `11_WORLD_MANAGER_UI.md` — desktop manager and in-app selector usable at ~120 Worlds.
13. `12_SECURITY_AND_FAILURE_MODES.md` — fail-closed rules and contamination defenses.
14. `13_TEST_AND_ACCEPTANCE_PLAN.md` — isolation, scale and production switch acceptance gates.
15. `14_MIGRATION_PLAN.md` — staged implementation from the baseline.
16. `15_OPERATIONS_RUNBOOK.md` — server deployment, build, promote, rollback, restore, health, retention, archive, purge.
...
20. `19_PRODUCTION_SCALE_120_WORLDS.md` — dedicated production/server scale contract.
21. `AI_LOCK.yaml` — machine-readable architecture lock.
22. `WORLD_SYSTEM_AI.instructions.md` — short drop-in instruction file for AI tools.
23. `examples/world-pack.example.yaml` — Seed Pack manifest example.
